...
export async function evaluateToolCorrectnessWithLLM(
  ...
  const itemResults = await Promise.all(
    CHECKLIST_ITEMS.map(async (checkDescription, itemIndex) => {
        ...
        try {
          ...
          const response = await client.createChatCompletion({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are validating tool correctness in conversations. You MUST respond with ONLY valid JSON - no markdown code blocks, no explanations, no additional text before or after. Your entire response must be parseable as JSON.'
              },
              {
                role: 'user',
                content: prompt
          ...
          if (attempt > 1) {
            console.log(`[LLMAJ Section 2] Item ${itemIndex} succeeded on attempt ${attempt}`);
          }
          ...
          lastError = error;
          console.error(`[LLMAJ Section 2] Item ${itemIndex} attempt ${attempt}/${maxRetries} failed:`, error.message);
      ...
      // All retries failed
      failedCallCount++;
      console.error(`[LLMAJ Section 2] Item ${itemIndex} failed after ${maxRetries} attempts`);
      return {
        checkDescription,
        passed: false,
